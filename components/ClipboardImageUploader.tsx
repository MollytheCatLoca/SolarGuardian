...
import { Upload, Save, Image as ImageIcon, X, ZoomIn, ZoomOut, RotateCwSquare, Clipboard } from 'lucide-react';

/**
 * Componente ClipboardImageUploader - Permite pegar imágenes del portapapeles
 */
export function ClipboardImageUploader() {
  ...
  // Preparar el área de pegado para recibir eventos globales
  useEffect(() => {
    const handlePaste = (e) => {
    ...
    // Agregar el listener para pegar imágenes
  ...
  // Función para activar la zona de pegado
  const activatePasteZone = (key) => {
    setCurrentKey(key);
  };
  
  // Función para subir una imagen desde archivo
    ...
    <div className="bg-[#0a0b14] border border-gray-800 rounded-lg p-6 max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Object.keys(images).map((key) => (
            <div
              ...
              onClick={() => activatePasteZone(key)}
              tabIndex={0}
              ref={currentKey === key ? pasteZoneRef : null}
            >
              ...
              <div 
                className={`p-6 flex-grow flex items-center justify-center cursor-pointer ${currentKey === key ? 'border-4 border-dashed border-purple-500' : ''}`} 
              ...
              >
                {images[key] ? (
                  ...
                  <div className="flex flex-col items-center text-center text-gray-500">
                    {currentKey === key ? (
                      <>
                        <Clipboard size={48} className="mb-2 opacity-70 text-purple-500" />
                        <p className="text-sm text-purple-400">
                          Haz CTRL+V para pegar una captura
                        </p>
                      </>
                    ) : (
                      <>
      ...
      <div className="mt-8 bg-gray-900 border border-gray-800 rounded-lg p-4">
        ...
        <ol className="list-decimal text-gray-400 pl-5 space-y-2">
          ...
          <li>Haz clic en una de las áreas de imagen y luego <strong>presiona CTRL+V</strong> para pegar tu captura</li>
          <li>También puedes hacer clic en "Pegar" después de copiar una imagen al portapapeles</li>
      ...
      <div className="mt-6 p-4 bg-blue-900/30 border border-blue-700 rounded-lg text-center">
        <p className="text-blue-300 font-medium mb-2">Consejo</p>
        <p className="text-gray-300 text-sm">
          Para hacer capturas de pantalla rápidamente, puedes usar <strong>Windows+Shift+S</strong> en Windows o 
          <strong>Command+Shift+4</strong> en Mac, luego selecciona el área y pégala directamente en cualquiera de los recuadros.
        </p>
      </div>
    </div>
  );
